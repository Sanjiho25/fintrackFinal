...
const BudgetCard: FC<BudgetCardProps> = ({ budget, categories, onEdit, onDelete }) => {
  ...
  const cardContent = (
    <Card className="w-full shadow-md hover:shadow-lg transition-shadow border-l-4 rounded-lg hover:bg-secondary/30"
          ...
          {!isSavings ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); onEdit(budget.id); }}>
                  <Edit className="mr-2 h-4 w-4" /> Edit Budget
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); onDelete(budget.id); }} className="text-destructive focus:bg-destructive/10 focus:text-destructive">
  ...
  return isSavings ? (
    ...
    <Link href={`/budget?id=${budget.id}`} className="block">
        {cardContent}
    </Link>
  );
};
