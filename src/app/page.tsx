...
import { PlusCircle, Target, Lightbulb, PiggyBank, Settings, BookOpen, AlertCircle, Wallet, BarChart3, Activity, UserCircle, Home as HomeIcon, Edit, Trash2, TrendingDown, Scale, FolderCog, DollarSign, CreditCard, ChevronDown, Check, Filter, MoreVertical, History, List, Calendar, Repeat, Bell } from "lucide-react";
...
import type { Transaction, Budget, Category, AppData, SavingGoal, TransactionType, MonthlyReport, NotificationSettings, Notification as AppNotification } from "@/types";
import { format, isToday, isYesterday, startOfMonth, subMonths, endOfMonth, parseISO, startOfDay, endOfDay, isBefore, startOfToday } from 'date-fns';
...
import { GeneralTips } from "@/components/general-tips";
...
import { scheduleDailyReminder, scheduleWeeklySummary, cancelAllNotifications, cancelNotification, scheduleGoalReminder, cancelGoalReminder, triggerTransactionNotification, initNotificationListeners, listPendingNotifications, checkPermissions, requestPermissions, DAILY_REMINDER_ID, WEEKLY_SUMMARY_ID } from "@/lib/notifications";
...
const budgetTourSteps = [
    {
        ...
        popover: {
            ...
            description: "Tap this button to create budgets for your spending categories. This is key to tracking your finances!",
            side: 'top' as any,
            align: 'end' as any,
        }
    }
...
export default function Home() {
  ...
  const generateMissingReports = React.useCallback(() => {
      if (!isLoaded || !user) return;
  
      setAppData(prevData => {
          const processedMonths = new Set(prevData.monthlyReports.map(r => r.month));
          
          // Get all unique months from transactions that are in the past
          const transactionMonths = new Set(
              prevData.transactions
                  .map(t => format(t.date, 'yyyy-MM'))
                  .filter(month => month < format(startOfToday(), 'yyyy-MM'))
          );
          ...
          transactionMonths.forEach(month => {
              if (!processedMonths.has(month)) {
                  // Found a past month with transactions but no report
                  const monthTransactions = prevData.transactions.filter(t => format(t.date, 'yyyy-MM') === month);
                  if (monthTransactions.length > 0) {
                      ...
                      const newReport: MonthlyReport = {
                          ...
                          expenseBreakdown: Object.entries(expenseBreakdownMap).map(([categoryId, data]) => ({ categoryId, ...data })),
                      };
          ...
          if (newReports.length > 0) {
              const updatedReports = [...prevData.monthlyReports, ...newReports].sort((a, b) => b.month.localeCompare(a.month));
              
              toast({
                  title: "Monthly Reports Saved",
                  description: `${newReports.length} new monthly summary/summaries have been generated and saved.`,
              });
  ...
  React.useEffect(() => {
    if (isLoaded && user) {
        ...
        const listenersRef = (window as any).__fintrack_notification_listeners_registered__;
        if (!listenersRef) {
            initNotificationListeners().catch(() => {});
            (window as any).__fintrack_notification_listeners_registered__ = true;
        }
        ...
        const handleNotifications = async () => {
            ...
            try {
                ...
                if (perm.display !== 'granted') {
                    ...
                    if (req.display !== 'granted') {
                        // Not permitted: cancel any pending and don't try to schedule
            ...
            const dailyReminderScheduled = localStorage.getItem('dailyReminderLastScheduled') === todayStr;
            const weeklySummaryScheduledThisWeek = parseInt(localStorage.getItem('weeklySummaryLastScheduledWeek') || '0', 10) === new Date().getDay();


            if (settings.dailyReminder && !dailyReminderScheduled) {
                const newNotif = await scheduleDailyReminder();
                if (newNotif) {
                    localStorage.setItem('dailyReminderLastScheduled', todayStr);
            ...
            if (settings.weeklySummary && !weeklySummaryScheduledThisWeek) {
                const newNotif = await scheduleWeeklySummary();
                 if (newNotif) {
                    localStorage.setItem('weeklySummaryLastScheduledWeek', String(new Date().getDay()));
                    newNotifications.push(newNotif);
                }
            } else if (!settings.weeklySummary) {
                await cancelNotification(WEEKLY_SUMMARY_ID);
            }
        ...
        // Ensure goal reminders are scheduled for existing goals (only if notifications enabled)
        (async () => {
            try {
                if (appData.notificationSettings?.enabled) {
                    const scheduledGoalsToday = localStorage.getItem('goalRemindersLastScheduled') === new Date().toDateString();
                    if (!scheduledGoalsToday) {
                        for (const g of appData.savingGoals) {
                            // scheduleGoalReminder will cancel and reschedule safely
                            const newNotif = await scheduleGoalReminder(g);
                            if (newNotif) {
                                setAppData(prev => ({ ...prev, notifications: [newNotif, ...prev.notifications] }));
                            }
                        }
                        localStorage.setItem('goalRemindersLastScheduled', new Date().toDateString());
                    }
                }
            } catch (e) {
                console.warn('Error scheduling existing goal reminders on startup:', e);
            }
        })();
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoaded, user, appData.notificationSettings]);
  ...
  React.useEffect(() => {
    ...
    React.useEffect(() => {
        if(currentTab === 'home' || currentTab === 'insights') {
            generateMissingReports();
        }
        setCurrentTab(initialTab);
    }, [initialTab, currentTab, generateMissingReports]);

  useDriverTour({
    tourId: 'welcome-tour-session',
    steps: welcomeTourSteps,
    condition: Boolean(isLoaded && user),
  });

  useDriverTour({
    tourId: 'budget-page-tour-session',
    steps: budgetTourSteps,
    condition: Boolean(isLoaded && user && currentTab === 'budgets' && (appData.monthlyIncome ?? 0) > 0),
  });
 ...
 React.useEffect(() => {
  ...
  const handleSaveTransaction = async (transactionData: Transaction) => {
    ...
    // Use a function for setAppData to ensure we have the latest state
    setAppData(prev => {
        ...
        let updatedSavingGoals = prev.savingGoals;
    ...
    const categoryLabel = targetSavingGoal ? targetSavingGoal.name : (getCategoryById(transactionData.category, categories)?.label ?? transactionData.category);

    if (isUpdate) {
        toast({ title: "Transaction Updated", description: `Transaction for ${categoryLabel} updated.` });
    } else {
        toast({ title: "Transaction Added", description: `${transactionData.type === 'income' ? 'Income' : (targetSavingGoal ? 'Allocation to' : 'Expense')} of ${formatCurrency(transactionData.amount)} logged for ${categoryLabel}.` });
        
        // Trigger notification if setting is enabled
        if (appData.notificationSettings.enabled && appData.notificationSettings.transactionNotifications) {
            await triggerTransactionNotification(transactionData, categoryLabel);
        }
    }
...
const openEditBudgetDialog = (budgetId: string) => {
 ...
 const handleSetIncome = () => {
  ...
  const totalMonetaryAllocatedToGoals = React.useMemo(() => {
    ...
    const handleAddOrUpdateGoal = async (goalData: Omit<SavingGoal, 'id' | 'targetAmount'> & { id?: string; targetAmount: number }) => {
        ...
        let toastMessageDescription = "";
        let goalToSave: SavingGoal | null = null;
        let isNewGoal = false;
        ...
        // Use a timeout to ensure state has updated before calling async notification logic
        setTimeout(async () => {
            if (goalToSave) {
                 const newNotif = await scheduleGoalReminder(goalToSave);
                 if (newNotif) {
                     setAppData(prev => ({
                         ...prev,
  ...
  return (
    <div className="flex flex-col flex-1 bg-background">
      <Tabs defaultValue={initialTab} value={currentTab} onValueChange={setCurrentTab} className="flex-grow flex flex-col">
                             ...
                             <>
                                ...
                                {savingGoals.length > 0 ? (
                                    savingGoals.map((goal, index) => {
                                        ...
                                        return (
                                            <Link href={`/goal?id=${goal.id}`} key={goal.id} className="block">
                                                <Card 
                                                    className="animate-slide-up cursor-pointer hover:bg-secondary/30" 
                                                ...
                                                >
                                                    <CardHeader className="flex flex-row items-start justify-between p-4 pb-2">
                                                        ...
                                                        <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                                                            <DropdownMenuItem onClick={(e) => { e.preventDefault(); openEditGoalDialog(goal); }}>
                                                                <Edit className="mr-2 h-4 w-4" /> Edit
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem onClick={(e) => { e.preventDefault(); setGoalToDelete(goal); }} className="text-destructive">
        ...
        {(currentTab === 'home' || currentTab === 'transactions') && (
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <PopoverTrigger asChild>
                                    <Button
                                        size="icon"
                                        className="rounded-full h-14 w-14 shadow-lg bg-primary hover:bg-primary/90 active:scale-95 transition-transform disabled:bg-muted disabled:cursor-not-allowed"
                            ...
                            {(!monthlyIncome || monthlyIncome <= 0) && (
                                <TooltipContent side="left" className="mb-2">
                                    <p>Set your income on the Home tab first.</p>
                                </TooltipContent>
                            )}
                        </Tooltip>
                    </TooltipProvider>
