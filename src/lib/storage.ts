...
import type { AppData, SavingGoalCategory, Category, MonthlyReport, NotificationSettings } from '@/types';
...
const defaultNotificationSettings: NotificationSettings = {
    enabled: false, // Default to false, user must opt-in
    dailyReminder: true,
    weeklySummary: true,
    transactionNotifications: false, // Default to false
};
  ...
  try {
    ...
    if (storedData) {
      ...
      const mergedData: AppData = {
        ...
        notificationSettings: { ...defaultNotificationSettings, ...parsedData.notificationSettings },
        notifications: parsedData.notifications ? parsedData.notifications.map((n: any) => ({ ...n, createdAt: parseISO(n.createdAt) })) : [],
      };
      ...
      // Add any custom categories from storedData that are not in defaults, excluding the removed default categories
      const excludedIds = ['housing', 'rent_mortgage', 'bills', 'water_bill', 'electric_bill', 'internet_bill', 'phone_bill', 'subscriptions_bills', 'public_transit'];
      mergedData.categories.forEach(storedCat => {
        if (!finalCategories.some(fc => fc.id === storedCat.id) && !excludedIds.includes(storedCat.id)) {
      ...
      mergedData.categories = finalCategories.filter(c => c.id !== 'income' && !excludedIds.includes(c.id)); // Remove conceptual 'income' and removed defaults
